...
export default function PostsSidebar({ categories, recentPosts }) {
  return (
    <div className="widget sticky top-[107px] mt-8 space-y-10 lg:mt-0">
      ...
      >
        <h4 className="border-b border-white border-opacity-20 pb-2 font-medium text-primary text-[18px]">
          Categories
        </h4>
        <ul className="styledlist mb-0 list-none pl-0">
          {categories.map((category) => (
            <li key={category.name}>
              <Link
                href={`/portfolio/category/${createSlug(category.name)}/1`}
        ...
        <ul className="mb-0 list-none pl-0">
          {recentPosts.map((post, index) => (
            <li key={index} className="mb-4 last:mb-0">
              <p className="mb-0">
                <Link
                  href={`/portfolio/postdetails/${post.slug}`}
