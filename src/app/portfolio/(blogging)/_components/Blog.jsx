  ...
  return (
    <article className="blog card p-4 md:p-5">
      <div className="blog-top relative mb-4">
        <Link
          href={`/portfolio/postdetails/${slug}`}
        ...
        >
          <Image
            ...
            height={289}
            width={600}
            alt="Saga V Brand Guidelines book displaying the cover, scalable typography, and the brand color palette." //Blog Image
            placeholder="blur"
      ...
      <div className="flex list-none gap-1.5 text-sm">
        {category.map((cat, i) => (
          <span key={i} className="after:content-[','] last:after:hidden">
            <Link href={`/portfolio/category/${createSlug(cat)}/1`} className=" hover:text-primary">
              {cat}
            </Link>
          </span>
        ))}
